...
### Added
- **insightfinder_project**: New `custom_setting` attribute (String/JSON, Optional, Computed) — arbitrary customer-specific settings (e.g. `{"nbc":{"global_commerce":{"flow":"enabled"}}}`). Maps to `customSetting`, which was previously absent from the resource despite being accepted and returned by the `watch-tower-setting` API. Modeled as raw JSON (like `email_setting`/`base_value_setting`) since its shape and keys are customer-defined and vary arbitrarily.
...
### Added
- **insightfinder_system_settings**: New `project_level_dampening_periods` set attribute inside `notifications_settings`, distinct from `project_level_dampening_windows`. Each entry has `project` (Required), `customer` (Optional, defaults to the provider username), and `duration` (Required). Maps to the `projectLevelDampeningPeriods` field in the `healthviewsetting` API, which was previously absent from the `HealthViewSetting` client struct and silently stripped for every system in the account whenever `notifications_settings` was applied for any single system.
...
### Added
- **insightfinder_project**: Four new optional attributes to match previously-missing API fields:
  - `model_match_threshold` (Number, Optional, Computed) — model match threshold for log pattern matching. Maps to `modelMatchThreshold`.
  - `model_keyword_segment_k` (Number, Optional, Computed) — number of keyword segments (K) used for the log model. Maps to `modelKeywordSegmentK`.
  - `disable_log_processing_flag` (Boolean, Optional, Computed) — disables log processing. Maps to `disableLogProcessingFlag`.
  - `incident_priority_cap_setting` (String/JSON, Optional, Computed) — caps for incident priority. Accepts a JSON-encoded object with `ticketCreationPriorityCap` and `suggestedPriorityCap` (both String). Maps to `incidentPriorityCapSetting` in the `watch-tower-setting` API.
- **insightfinder_metric_project**: New `incident_priority_cap_setting` attribute (String/JSON, Optional, Computed), same shape and API mapping as above.

## [1.10.2] - 2026-07-15

### Fixed
- **insightfinder_slack**: `Delete` now locates the integration to remove by matching system/username/webhook/channel against the list API (mirroring `Update`) instead of trusting the possibly-stale `id` stored in state, then removes it via `POST /api/external/v1/service-integration` with `operation=delete` and `service_id=Slack:{account}:SLACK`.
- **insightfinder_servicenow**: Fixed `Delete` sending the wrong `serviceProvider` value (`PagerDuty`, a copy-paste leftover) on the `POST /api/external/v1/service-integration` delete call — now correctly sends `ServiceNow`.

## [1.10.1] - 2026-07-07

### Added
- **insightfinder_slack**: Two new optional top-level attributes:
  - `priority_upgrade_channel` (String, Optional, Computed) — Slack channel to notify when an incident's priority is upgraded.
  - `priority_upgrade_webhook` (String, Sensitive, Optional, Computed) — Slack webhook URL to notify when an incident's priority is upgraded.
  - Sent as top-level `priorityUpgradeChannel`/`priorityUpgradeWebhook` form fields on create/update, and read back from the `priorityUpgradeChannel`/`priorityUpgradeWebhook` keys nested inside the API's `configs` JSON blob (alongside `projectConfigs`).
...
### Fixed
- **insightfinder_metric_project**: `metric_alert_settings` entries are now stored in alphabetical order by `component_name` (global entry first) when reading from the API, eliminating spurious plan diffs caused by the server returning component-level settings in a different order than the configuration.
